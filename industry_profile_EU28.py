...
class Industry(object):
    ...
    def analysis_byregions(self,tradetype, region):
        ...
        if region == 'EU28': region_cty_name=EU28_cty_name
    ...
    def export_to_excel(self, numberofdecimal, periodsdata=False, alldata=False):
        ...
        if self.needsymbol:
            file_path = folder_path+"/"+self.periods[0]+"-"+self.periods[-1]+'_denoted_EU28'+"/"+self.currency+"/"+self.money
        if not self.needsymbol:
            file_path = folder_path+"/"+self.periods[0]+"-"+self.periods[-1]+'_notdenoted_EU28'+"/"+self.currency+"/"+self.money
        ...
        # by region
        # EU
        self.DXbyEU28.to_excel(writer4, 'DXbyEU28', float_format=f"%.{numberofdecimal}f", startrow=1)
        self.TXbyEU28.to_excel(writer4, 'TXbyEU28', float_format=f"%.{numberofdecimal}f", startrow=1)
        self.RXbyEU28asDestination.to_excel(writer4, 'RXbyEU28asDestination', float_format=f"%.{numberofdecimal}f", startrow=1)
        self.RXbyEU28asOrigin.to_excel(writer4, 'RXbyEU28asOrigin',float_format=f"%.{numberofdecimal}f", startrow=1)
        self.IMbyEU28asConsignment.to_excel(writer4, 'IMbyEU28asConsignment',float_format=f"%.{numberofdecimal}f", startrow=1)
        self.IMbyEU28asOrigin.to_excel(writer4, 'IMbyEU28asOrigin',float_format=f"%.{numberofdecimal}f", startrow=1)
        self.TTbyEU28.to_excel(writer4, 'TTbyEU28', float_format=f"%.{numberofdecimal}f", startrow=1)
        ...
        try:
            ...
            ex.addtitle(self.excelfile_name, self.name, EU_name="EU(28)")
...
if __name__ == '__main__':
    ...
    for k, v in industrycode.items():
        ...
        #table 3 by region
        # EU
        industrycode[k]['class'].DXbyEU28=industrycode[k]['class'].analysis_byregions('DX','EU28')
        industrycode[k]['class'].TXbyEU28=industrycode[k]['class'].analysis_byregions('TX','EU28')
        industrycode[k]['class'].RXbyEU28asDestination=industrycode[k]['class'].analysis_byregions("RX",'EU28')
        industrycode[k]['class'].RXbyEU28asOrigin=industrycode[k]['class'].analysis_byregions("RXbyO",'EU28')
        industrycode[k]['class'].IMbyEU28asConsignment=industrycode[k]['class'].analysis_byregions("IM",'EU28')
        industrycode[k]['class'].IMbyEU28asOrigin=industrycode[k]['class'].analysis_byregions("IMbyO",'EU28')
        industrycode[k]['class'].TTbyEU28=industrycode[k]['class'].analysis_byregions("TT",'EU28')
