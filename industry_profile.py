...
from BSO.hstositc import get_sitc_name
...
class Industry(object):
    ...
    def analysis_byproducts(self, tradetype, codetypeanddigit1, codetypeanddigit2=None, codetypeanddigit3=None):
        ...
        """
        # add one column for product name
        print('HI')
        print(self.codetypeanddigit)
        print(data)
        for col in self.codetypeanddigit:
            print(col)
            if col=='SITC-3':
                data.loc[:,col+'_product_name'] = data.index.map(get_sitc_name())
            elif col=='SITC-5':
                data.loc[:,col+'_product_name'] = data.index.map(get_sitc_name(sheet_name="sitc5"))
        """
        return byproduct
